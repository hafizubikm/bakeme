<?php
/* SVN FILE: $Id:model.php 7219 2008-06-20 00:42:08Z gwoo $ */
/**
 ...
 * @lastmodified	$Date:2008-06-20 02:42:08 +0200 (vr, 20 jun 2008) $
 ...
 */
	function __interactive() {
		...
		if (in_array($useTable, $this->__tables)) {
			App::import('Model');
			$this->tempModel = new Model(array('name' => $currentModelName, 'table' => $useTable, 'ds' => $useDbConfig));

			$fields = $this->tempModel->schema();
		...
		if (array_search($useTable, $this->__tables) !== false && (low($wannaDoValidation) == 'y' || low($wannaDoValidation) == 'yes')) {
			$validate = $this->doValidation($this->tempModel);
		}
		...
		if ((low($wannaDoAssoc) == 'y' || low($wannaDoAssoc) == 'yes')) {
			$associations = $this->doAssociations($this->tempModel);
		}
	...
	function bake($name, $associations = array(),  $validate = array(), $primaryKey = 'id', $useTable = null, $useDbConfig = 'default') {
		...
		if (is_array($validate) && $validateCount > 0) {
			...
			for ($i = 0; $i < $validateCount; $i++) {
				$out .= "\t\t'" . $keys[$i] . "' => array(array('rule' => array('" . $validate[$keys[$i]] . "')))";
		...
		if(!empty($this->tempModel)){
			$fields = $this->tempModel->schema();
			//parent_id, lft & rght field?
			if(array_key_exists('parent_id', $fields) && array_key_exists('lft', $fields) && array_key_exists('rght', $fields)){
				$out .= "\tvar \$actsAs = array('Tree');\n";
			}
			//password field?
			if (array_key_exists('password', $fields)) {
				$out .= "\t function beforeSave(){\n";
				$out .= "\t\tif(!empty(\$this->data['{$name}']['new_password'])){\n";
				$out .= "\t\t\t\$this->data['{$name}']['password'] = Security::hash(\$this->data['{$name}']['new_password'], null, true);\n";
				$out .= "\t\t}\n";
				$out .= "\t\tif(!empty(\$this->data['{$name}']['your_password'])){\n";
				$out .= "\t\t\t\$this->data['{$name}']['password'] = Security::hash(\$this->data['{$name}']['your_password'], null, true);\n";
				$out .= "\t\t}\n";
				$out .= "\t\treturn parent::beforeSave();\n";
				$out .= "\t}\n";
				$out .= "\n";
			}
		}
