<?php
/* SVN FILE: $Id:view.php 6574 2008-03-15 05:45:43Z gwoo $ */
/**
 ...
 */
	function execute() {
		...
		if (isset($this->args[0])) {
			...
			if($this->args[0] == 'all')
			{
				$controllers = $this->Controller->listAll('default', 'Controllers');
				foreach($controllers as $controllerName)
				{
					...
					if (App::import('Model', $currentModelName)) {
						$this->controllerName = Inflector::camelize($controllerName);
						...
						if ($vars) {
							...
							foreach ($methods as $method) {
								if ($method{0} != '_' && !in_array(low($method), array_merge($protected, array('delete', $adminDelete, $adminRoute.'_undelete', $adminRoute.'_hard_delete')))) {
									$content = $this->getContent('views/' . $method, $vars);
 ...
 */
	function __interactive() {
		...
		if (low($wannaDoScaffold) == 'y' || low($wannaDoScaffold) == 'yes') {
			...
			if ($vars) {
				foreach ($actions as $action) {
					$content = $this->getContent('views/' . $action, $vars);
 ...
 */
	function __loadController() {
		...
		return compact('modelClass', 'primaryKey', 'displayField', 'singularVar', 'pluralVar',
				...
				'controllerPath', 'schema', 'modelObj');
	}
/**
 ...
 */
	function getContent($template = null, $vars = null) {
		...
		$template = $template_split[count($template_split) - 1];
		$subdir = '';
		for($i = 0; $i < count($template_split)-1; $i++){
			if(!empty($subdir)) $subdir .= DS;
			$subdir .= $template_split[$i];
		}
		//
		$action = $template;
		...
		*/
		...
		foreach ($this->Dispatch->shellPaths as $path) {
			$templatePath = $path . 'templates' . DS . $subdir . DS .Inflector::underscore($template).'.ctp';
