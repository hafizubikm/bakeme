<?php
	 ...
	 *
	 * Updated 12/2008 by Folkert Hielema to include search functionality and to match updates to Twitter API.
	 * 
	 * @author	Aral Balkan
	 * @copyright	2007-2009 Aral Balkan. All Rights Reserved. 
	...
	**/
	class Twitter extends BaseService
	{
		 ...
		 * @author Aral Balkan
		 **/
		function friendsTimeline($user, $pass, $id = NULL, $since = NULL, $count = 50,$page = 0)
		{
			$url = 'twitter.com/statuses/friends_timeline.json';
			$since = (empty($since)) ? NULL : $since;
			$vars = array('id' => $id, 'since' => $since, 'count' => $count, 'page' => $page);
		 ...
		 * @author Aral Balkan
		 **/
		function userTimeline($user, $pass, $id = NULL, $count = 50, $since = NULL)
		{
		 ...
		 * @param (optional) id for the message to reply to
		 * @return 	(array)	Success/failure message.
		 * 
		 * @author 	Aral Balkan
		 * @changed by Folkert Hielema, added optional $reply_id
		 **/
		...
		{
			if($id !== NULL)
				$url = "twitter.com/statuses/friends/$id.json";
			else
		 ...
		 * @param	(str) Password.
		 * @param	(str) user A
		 * @param	(str) user B (Optional)
		 * @return 	(array)	List of friends.
		 * @author Aral Balkan
		 **/
		function isFriend($user, $pass, $userA, $userB)
		{
			$vars = array('user_a' => $userA, 'user_b' => $userB);
			$url = "twitter.com/friendships/exists.json";
		...
		/**
		 * Returns the remaining number of API requests available to the authenticating user before the API limit is reached for the current hour. Calls to rate_limit_status require authentication, but will not count against the rate limit.
		 *
		 * @param	Your username.
		 * @param	Your password.
		 *
		 * @return Number of API requests.
		 * @author Aral Balkan
		 **/
		function rateLimitStatus( $user, $pass)
		{
			$url = 'twitter.com/account/rate_limit_status.json';
			$response = $this->_jsonCall($url, NULL, 'GET', $user, $pass);
			
			return $response;
		}
		/**
		 * Updates the location attribute of the authenticated user, as displayed on de side of their profiles and returned in various API Methods.
		 *
		 * @param	Users login name.
		 * @param	Users password.
		 * @param Users location
		 * @return The same object as with showUser, with info and last post
		 * @author Folkert Hielema
		 **/
		function update_location( $user, $pass, $location)
		{
			$url = 'twitter.com/account/update_location.json';
		...
		/**
		 * Sets which device Twitter delivers updates to for the authenticated user. Sending none as device parameter will disable IM or SMS updates.
		 *
		 * @param	Users login name.
		 * @param	Users password.
		 * @param (required) must be sms, im or none.
		 * @return The same object as with showUser, with info and last post
		 * @author Folkert Hielema
		 **/
		function update_delivery_device( $user, $pass, $device)
		{
			$url = 'twitter.com/account/update_delivery_device.json';
			$vars = array('device' => $device);
			$response = $this->_jsonCall($url, $vars, 'POST', $user, $pass);
			
			return $response;
		}
		/**
		 * Search twitter implementation from twitter.com
		 * @param (str) SearchString
		 * @param (str) (optional) Language short tag like 'en', 'nl' etc
		 * @param (int) (optional)  rpp - The number of tweets to return per page, max = 100
		 * @param (str) (optional) since_id - return tweets with a status_id greater than the given
		 * @param (str) (optional) geocode - return tweets within the given radius of the given latitude/longtitude. Location is taken from users twitter profile
		 * @param (bool) (optional) show_user - When 'true', adds "<user>:" to the beginning of the tweet, useful for readers that do not display Atom's author field, default = false
		 * @return (array) Messages found for searchString
		 * @author Folkert Hielema
		 **/
		function search( $searchString,  $lang = NULL, $rpp = 20, $since_id = NULL, $geocode = NULL, $show_user = false )
		{
			$url = "search.twitter.com/search.json";
			$vars = array('q' => "$searchString", 'lang' => $lang, 'rpp' => $rpp, 'since_id' => $since_id, 'geocode' =>$geocode, 'show_user' =>$show_user);
